...
const getAllGamesAPI = async (num) => {
    ...
    while(counter <= num) {
        ...
        api.data.results.map(vg => allGamesArray.push({  
            id: vg.id,
            name: vg.name,
...
router.get('/', async (req, res) => {
    try {
        ...
        if (!name) {
            ...
            const filteredGamesApi = allVideogamesAPI.filter(vg => vg.name.toLowerCase().includes(name.toLowerCase()));

            const filteredDb = db.filter(vg => vg.name.toLowerCase().includes(name.toLowerCase()))
...
router.post('/', async (req, res) => {
    try {
        ...
        if(!name || !description || !image || !platforms || !genres) return res.status(400).send('Should type all data');
        else if(!gameExists) {
            ...
            return res.status(200).send('The game has been created with successfully')
        } else {
